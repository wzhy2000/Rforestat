...
############## (5) 模型性能 ######################
pre.gnls2 <- predict(model.gnls2, newdata = test.data)

FittingEvaluationIndex(pre.gnls2, test.data$H)

############### (6) 可视化 ###############

pdf("9.9fit.plot.pdf", width = 8, height = 8, family = "GB1")
par(mar = c(5, 5, 4, 2), mgp = c(3.5, 1, 0))
plot(pre.gnls2, test.data$H, xlab = "拟合树高/cm", ylab = "树高/cm", las = 1, 
     pch = 16, col = "black", cex = 1, 
     cex.lab = 2, cex.axis = 2)
dev.off()

# 训练集残差图
res.train <- residuals(model.gnls2, type = "response")
pdf("9.10Residuals.train.pdf", width = 8, height = 8, family = "GB1")
par(mar = c(5, 5, 4, 2), mgp = c(3.5, 1, 0), mfrow = c(1, 1))
plot(fitted(model.gnls2), res.train, xlab = "拟合值", 
...
# 测试集残差图
res.test <- test.data$H - pre.gnls2
pdf("9.11Residuals.test.pdf", width = 8, height = 8, family = "GB1")
par(mar = c(5, 5, 4, 2), mgp = c(3.5, 1, 0))
plot(pre.gnls2, res.test, xlab = "拟合值", 
     ylab = "残差", pch = 16, col = "black", cex = 1, 
     cex.lab = 2, cex.axis = 2)
abline(h = 0, col = "red")
dev.off()
