...
plot(data$fit.CW, data$CW, xlab = "拟合冠幅(m)", ylab = "冠幅(m)", col = "black", cex.axis = 2.5, cex.lab = 2.5)
dev.off()

# 绘制模型的拟合值与残差图
pdf("图10.1b.pdf", width = 8, height = 6, family = "GB1")
par(mar = c(5, 5.5, 4, 2), mgp = c(3.5, 1, 0))
plot(fitted(model.CW.Pt), residuals(model.CW.Pt), xlab = "拟合冠幅(m)", ylab = "残差(m)",  cex.axis = 2.5, cex.lab = 2.5)
abline(h = 0, col = "red")
dev.off()
...
par(mar = c(5, 5.5, 4, 2), mgp = c(3.5, 1, 0))
plot(fitted(model.CW.Pt.varPower ), residuals(model.CW.Pt.varPower), xlab = "拟合冠幅(m)", ylab = "残差(m)",  cex.axis = 2.5, cex.lab = 2.5)
abline(h = 0, col = "red")
dev.off()
