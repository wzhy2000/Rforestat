...
plot(larch$D,larch$CW,xlim=xlims, col="darkgrey",xlab="胸径(cm)",ylab="冠幅(m)", cex.lab = 2.5, cex.axis = 2.5)
...
summary(model.ns)
print( attr( ns(larch$D, df = 4), "knots") )

########## 4.光滑样条模型 #################
model.ss <- with(larch, stats::smooth.spline(D, CW, cv = TRUE))
print(model.ss)
