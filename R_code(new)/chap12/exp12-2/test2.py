...
class PSO():
    ...
    def function(self, X):
        ...
        # 假设 self.PLOT 是样地 ID（如 [1, 1, 1, 2, 2, 3, ...]）
        unique_plots = np.unique(self.PLOT)  # 找到所有唯一的样地编号
        mu_p = 0  # `p` 的总体均值，可以调整
        sigma_p = 0.67  # `p` 的随机效应标准差，可以调整
        # 为每个样地生成一个随机的 p
        plot_effects = {
            plot: np.random.normal(mu_p, sigma_p)
            for plot in unique_plots
        }
        for i in range(len(self.CW)):
            p = plot_effects[self.PLOT[i]]  # 根据样地 ID 选取 p

            # p = self.PLOT[i]

            numerator = ((a1 + p) + a2 * self.CLR[i])
...
with open('birch_subset.csv', 'r') as csvfile:
